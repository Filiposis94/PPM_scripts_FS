	...
	return (
		<div className="settings">
			...
			<button
				type="button"
				className="button"
				onClick={() => generateRandomTactics(mainTactics, setRandomizedMain)}
			>
				Generate main
			</button>
			<button
				...
				className="button"
				onClick={() =>
					generateRandomTactics(secondaryTactics, setRandomizedSecondary)
				}
			>
				Generate secondary
			</button>
			{randomizedMain.length > 0 && (
				<p>
					<b>Main:</b>{" "}
					{randomizedMain.map((t) => (
						<span key={t}>{t}</span>
					))}
				</p>
			)}
			{randomizedSecondary.length > 0 && (
				<p>
					<b>Secondary:</b>{" "}
					{randomizedSecondary.map((t) => (
						<span key={t}>{t}</span>
					))}
				</p>
			)}
