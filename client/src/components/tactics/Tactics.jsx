...
function Tactics(props) {
	...
	async function handleSubmit() {
		try {
			...
			const res = await axios.get(
				`/api/v1/tactics?teamId=${settings.teamId}&numOfDays=${settings.numOfDays}&socketId=${props.socketId}`
			)
	...
	React.useEffect(() => {
		async function fetchData() {
			try {
				const res = await axios.get(`/api/v1/tactics/next-opponent`)
				setSettings((prevSettings) => {
		...
		<div>
			...
			{!isLoading && (
				<TacticsSettings
					...
					settings={settings}
					isLoadingNextOp={isLoadingNextOp}
				/>
			)}
			{Object.keys(tacticsResults).length > 0 && <h3>Výsledek</h3>}
			{Object.keys(tacticsResults).length > 0 && (
				<TacticsResults tacticsResults={tacticsResults} />
			)}
		</div>
	)
}
