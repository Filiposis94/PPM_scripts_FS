...
function FriendlyMatch(props) {
	...
	for (let i = 0; i < availableMatches.length; i++) {
		if (
			availableMatches[i].capacity.includes("20000") &&
			availableMatches[i].sP === 80 &&
			availableMatches[i].bP > 18
		) {
	...
	function handleSelect(clickedDate) {
		setAvailableDates((prevAvailableDates) => {
			const newDates = prevAvailableDates.map((date) => {
				return clickedDate === date.value
	...
	async function handleSubmit() {
		try {
			if (
				availableDates.filter((date) => date.isSelected === true).length > 0
			) {
				setIsLoading(true)
				const joinedDates = availableDates
					.filter((date) => date.isSelected)
					.map((date) => date.value)
					.join(",")
				const res = await axios.get(
					`/api/v1/friendly-matches/?dates=${joinedDates}&tk=${settings.tk}&socketId=${props.socketId}&moreData=${settings.moreData}`
				)
	...
	React.useEffect(() => {
		async function fetchData() {
			try {
				const res = await axios.get(`/api/v1/friendly-matches/dates`)
				setAvailableDates(
					res.data.map((date) => {
						return {
							value: date,
	...
	// ELEMENTS
	const matchElements = availableMatches.map((match) => (
		<Match key={match.id} data={match} settings={settings} />
	))
	return (
		<div>
				...
				<div>
					<p className="settings">
						<button
							...
							className="button"
							onClick={() => {
								handleWindowOpening(awayMatches)
							}}
						>
							Open Away ({awayMatches.length})
						</button>
						<button
							type="button"
							className="button"
							onClick={() => {
								handleWindowOpening(homeMatches)
							}}
						>
							Open Home ({homeMatches.length})
						</button>
					</p>
					<table>
						<thead>
