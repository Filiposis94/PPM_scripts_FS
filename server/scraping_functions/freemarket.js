...
const scrapeFreeMarket = async (cz, socket, offset) => {
	...
	await page.type("input[name=contract_from]", `${8 + offset}`)
	await page.type("input[name=contract_to]", `${8 + offset}`)
	await page.type("input[name=index_skill_from]", `${cz}`)
	await page.click(
		"#filter_market > form > table:nth-child(5) > tbody > tr > td:nth-child(1) > button"
	)
	await page.waitForNetworkIdle()
	// GETTING UNIQUE ID OF SEARCH FOR PAGINATION
	const idOfSearch = await page.evaluate(() => {
		return document
			.querySelector("center:nth-child(6) > div > ul > li:nth-child(2) > a")
			.href.split("-")[1]
	})
	...
	do {
		...
		const tempArray = await page.evaluate(() => {
			const infoArray = []
			const allRows = document.querySelectorAll("#table-1 > tbody > tr")
			for (let i = 0; i < allRows.length; i++) {
				// for(let i = 1; i<2; i++){ //DEV
				// Scaning each player
				const name = allRows[i].querySelector(`td.name > a.link_name`).innerText
				const link = allRows[i].querySelector(`td.name > a.link_name`).href
				const age = allRows[i].querySelector(`td[title=Věk]`).innerText
				const sv = allRows[i].querySelector(
					`td[title='Sportovní výdrž']`
				).innerText
				const bra = allRows[i]
					.querySelector(`td[title=Brána]`)
					.innerHTML.split("<")[0]
				const obr = allRows[i]
					.querySelector(`td[title=Obrana]`)
					.innerHTML.split("<")[0]
				const uto = allRows[i]
					.querySelector(`td[title=Útok]`)
					.innerHTML.split("<")[0]
				const str = allRows[i]
					.querySelector(`td[title=Střela]`)
					.innerHTML.split("<")[0]
				const nah = allRows[i]
					.querySelector(`td[title=Nahrávka]`)
					.innerHTML.split("<")[0]
				const tec = allRows[i]
					.querySelector(`td[title=Technika]`)
					.innerHTML.split("<")[0]
				const agr = allRows[i]
					.querySelector(`td[title=Agresivita]`)
					.innerHTML.split("<")[0]
				const zku = allRows[i].querySelector(`td[title=Zkušenost]`).innerText
				const prs = allRows[i].querySelector(
					`td[title='Preferovaná strana']`
				).innerText
		...
		await page.goto(
			`https://hockey.powerplaymanager.com/cs/trh.html?data=${marketPage}-${idOfSearch}`,
	...
	for (let i = 0; i < allPlayers.length; i++) {
		...
		const teamLink = await page.evaluate(() => {
			// Last owner trade
			const lastOwner = document.querySelectorAll(".name")[1]
			if (lastOwner) {
				return lastOwner.childNodes[2].href
			} else {
				return document.querySelector(
					"div.white_box.left_corner_none > table:nth-child(4) > tbody > tr:nth-child(1) > td.tr0td2 > a"
				).href
			}
		})
		...
		const profileLink = await page.evaluate(() => {
			const profile = document.querySelector(
				"div.h1_header > div:nth-child(1) > div:nth-child(4) > a"
			)
		...
		if (profileLink !== "Bez manažera") {
			...
			const lastLogin = await page.evaluate(() => {
				return document.querySelectorAll(
					".table_profile > tbody > tr:nth-child(1)"
				)[1].innerText
			})
