	...
	do {
		...
		const tempArray = await page.evaluate(() => {
			...
			for (let i = 0; i < allRows.length; i++) {
				...
				const time = allRows[i]
					.querySelectorAll(`td.name > div`)[0]
					.innerText.split(" (")[0]
				const price = allRows[i]
				...
				const att1 = allRows[i]
					.querySelectorAll(`td`)[5]
					.innerHTML.split("<")[0]
				const att2 = allRows[i]
		...
		await page.goto(
			`https://hockey.powerplaymanager.com/cs/top-prodeje-zamestnancu.html?data=lastday-${marketPage}`,
