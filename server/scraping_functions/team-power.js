	...
	for (let i = 0; i < teams.length; i++) {
		...
		const teamData = await page.evaluate(() => {
			const power = document.querySelector(
				"div.profile_center_column > table:nth-child(1) > tbody > tr:nth-child(5) > td.tr0td2 > strong"
			).innerText
			const name = document.querySelector(
				"div.main_content > div.column_center_full > div.white_box.left_corner_none > div.profile_center_column > table:nth-child(3) > thead > tr > td.left_align"
			).innerText
			const league = document.querySelector(
				"body > div:nth-child(5) > div.h1_div > div.h1_header > div:nth-child(1) > div:nth-child(3) > a"
			).innerText
			return {
				power,
				name,
