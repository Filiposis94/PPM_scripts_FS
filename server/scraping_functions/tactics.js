...
const scrapeTactics = async (startDate, numOfDays, teamId, socket) => {
	...
	for (let i = 1; i <= numOfDays; i++) {
		...
		const dataCalendar = await page.evaluate(() => {
			const dayOfSeason = document
				.getElementById("day_in_season")
				.innerText.split(" ")[1]
			const matchType = document.querySelector(
				"div.calendary_center > span:nth-child(3)"
			).innerText
			const urlMatch = document.querySelector("div.button.report_icon > a").href
		...
		const forfeit = await page.evaluate(() => {
			if (
				document.querySelector(
					"#top > div.white_box > div.report_header_content > div.report_head_content.info > em > strong > span"
				)
			) {
				return true
			} else {
		...
		const dataMatch = await page.evaluate((teamId) => {
			...
			let ppTactics
			let pkTactics
			const homeTeamUrl = document.querySelector(
				"#top > div:nth-child(12) > table > tbody > tr:nth-child(1) > td.left_align.tr1td1 > a"
			).href
			const sOindicator = document
				.querySelector(".info")
				.innerText.includes("Po samostatných nájezdech")
			// IF Shoot Out
			if (sOindicator) {
		...
		// FINAL DATA
		finalData.push({
			day: dataCalendar.dayOfSeason,
			matchType: dataCalendar.matchType,
			link: dataCalendar.urlMatch,
			mainTactics: dataMatch.mainTactics,
			ppTactics: dataMatch.ppTactics,
			pkTactics: dataMatch.pkTactics
		})
	}
	// JOINING DATA AND SENDING FINAL OBJECT
	const mainAr = finalData
		.map((data) => data.mainTactics)
		.join(" / ")
		.split(" / ")
	const ppAr = finalData
		.map((data) => data.ppTactics)
		.join(" / ")
		.split(" / ")
	const pkAr = finalData
		.map((data) => data.pkTactics)
		.join(" / ")
		.split(" / ")
